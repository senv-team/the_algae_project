...
## plot the geojson regions over a world map for checking
  world = gpd.read_file(gpd.datasets.get_path('naturalearth_lowres')) # load world map for context
...
def plot_data(region_name, data, latitude, longitude, start_date, end_date):
  ...
  ax.set_aspect('equal')
  plt.pcolormesh(longitude, latitude, data, shading='auto', vmin=0, vmax=0.75)  # Set vmax to 0.75 for colorbar maximum
...
locs_dir = "./locs/"
locs_files = [os.path.join(locs_dir, file) for file in os.listdir(locs_dir) if file.endswith(".geojson")] # find all the geojson files in a directory
...
for data_info in tqdm(data_list, desc='Cropping data to region of interest'): # loop through all the data, and crop it to the region of interest
  ...
  for i, gdf in enumerate(gdf_list):
    ...
    data_crop_list.append({"data": chlor_a_crop, "latitude": latitude_crop, "longitude": longitude_crop, "start_date": data_info["start_date"], "end_date": data_info["end_date"], "region_name": region_name})
...
for data_info in data_crop_list:
  ...
  # plot the data
  plot_data(region_name, data_info['data'], data_info['latitude'], data_info['longitude'], data_info['start_date'].strftime('%Y%m%d'), data_info['end_date'].strftime('%Y%m%d'))
